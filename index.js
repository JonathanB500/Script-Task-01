...
const generateDocumentSheet = async (doc, data) => {
  ...
  let widthShift = 100;
  if (data.thumbnail) {
    description = doc.splitTextToSize(data.thumbnail["alt_text"], 180);
    heightShift =
  ...
  doc.addImage(image, "JPEG", 20, 65, widthShift, heightShift);
  ...
  doc.text(data["place_of_origin"] ?? "No origin place", 20, 100 + heightShift);
  ...
  try {
    ...
    if (Array.isArray(data)) {
      for (const art of data) {
        await generateDocumentSheet(doc, art);
      }
    } else {
      await generateDocumentSheet(doc, data);
    }
